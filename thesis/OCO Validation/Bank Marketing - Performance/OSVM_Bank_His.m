...
load bankadd
load bankaddfull     
Xval = (Xval-mean(Xval))./sqrt(var(Xval));                       % Load data
X = (X-mean(X))./sqrt(var(X));              % Normalize data
[n,m] = size(X);                            % Data size
% idx = randperm(n);                          % Randomize order
vt = 0;
Xval = X(vt*n/10+1:(vt+1)*n/10,:); Yval = Yval(vt*n/10+1:(vt+1)*n/10);
X(vt*n/10+1:(vt+1)*n/10,:)=[];Y(vt*n/10+1:(vt+1)*n/10)=[];
[n,m] = size(X);
%% Pipeline options
global print conv store vis vis2 evalper dynsig
dynsig = 1;             % Dynamic kernel width
evalper = 1;            % Evaluate iteration performance
...
%% Parameters
% LDA
D = 20;                  % Output dimension
% GMM
selsize = 5000;         % Datapoints taken for GMM
mp = 150;               % Number of fitted distributions
% SVM
epoch = 1;
eta = 1;                % Learning rate
...
while ep < epoch
    ...
    for t = 1:n
        ...
        if evalper
           [Yp,~] = svm_predict(a,b,K,Xval*G,XC,YC,0);
           GE(t+ep*n) = sum(sign(Yp)==Yval)*100/length(Yval);
        end
        if dynsig
